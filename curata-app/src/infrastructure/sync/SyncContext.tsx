...
export function SyncProvider({ children }: { children: React.ReactNode }) {
    ...
    const triggerSync = async () => {
        ...
        setIsSyncing(true);
        try {
            // Guard: não executar sync se as credenciais do Supabase não estiverem configuradas
            if (!isSupabaseConfigured) {
                console.log('[SyncContext] Supabase não configurado — sync ignorado.');
                setIsSyncing(false);
                return;
            }

            // Guard: não executar sync se não houver usuário autenticado no Supabase
